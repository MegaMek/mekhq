...
public class CommandGenerationLabel extends JLabel {
     ...
     */
    public CommandGenerationLabel(String name) {
        this(name, null, false);
    }

    /**
     * Same as {@link #CommandGenerationLabel(String)} but lets the caller suppress the tooltip lookup.
     *
     * @param name      the bundle-key suffix
     * @param noTooltip when {@code true}, no tooltip is set on the label
     */
    public CommandGenerationLabel(String name, boolean noTooltip) {
     ...
     * @param noTooltip      when {@code true}, no tooltip is set on the label
     */
    public CommandGenerationLabel(String name, @Nullable Integer customWrapSize, boolean noTooltip) {
        super(String.format("<html>%s</html>",
              getTextAt(getCommandGenerationResourceBundle(), "lbl" + name + ".text")));
