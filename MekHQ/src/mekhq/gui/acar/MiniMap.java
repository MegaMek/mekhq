...
public class MiniMap extends JPanel implements OverlayPainter {
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (var blip : removedUnits) {
            ...
            g.drawRect(p1[0] - 6 + xOffset, p1[1] - 6 + yOffset, 13, 13);

            var delta = game.getCurrentRound() + 1 - blip.round;
            var alpha = delta <= 0 ? 255 : (int) (blip.color.getAlpha() / (double) delta);
            var newColor = new Color(blip.color.getRed() / 2,
                blip.color.getGreen() / 2,
                blip.color.getBlue() / 2,
                alpha);
