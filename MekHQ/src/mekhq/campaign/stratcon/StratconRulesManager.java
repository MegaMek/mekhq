...
public class StratconRulesManager {
    ...
    /**
     * Generates a StratCon scenario. This is a utility method that allows us to generate a scenario quickly without
     * specifying track state and scenario template.
     *
     * @param campaign The current campaign.
    ...
    /**
     * Generates a new StratCon scenario using advanced configuration. It provides a scenario based on a given campaign,
     * contract, track, template. This is meant for scenario control on a higher level than the overloading methods.
     *
     * @param campaign              The current campaign.
     * @param contract              The contract associated with the scenario.
     * @param track                 The {@link StratconTrackState} the scenario should be assigned to, or {@code null}
     *                              to select a random track.
     * @param scenarioCoords        The {@link StratconCoords} where in the track to place the scenario, or {@code null}
     *                              to select a random hex. If populated, {@code track} cannot be {@code null}
     * @param template              A specific {@link ScenarioTemplate} to use for scenario generation, or {@code null}
     *                              to select scenario template randomly.
     * @param allowPlayerFacilities Whether the scenario is allowed to spawn on top of player-allied facilities.
     * @param daysTilDeployment     How many days beyond current date until the scenario, or {@code null} to pick a
    ...
    /**
     * Adds a {@link StratconScenario} to the specified contract. This scenario is cloaked so will not be visible until
     * the player uncovers it. If no {@link StratconTrackState} or {@link ScenarioTemplate} is provided, random one will
     * be picked.
     *
     * @param campaign              The current campaign.
     * @param contract              The {@link AtBContract} associated with the scenario.
     * @param trackState            The {@link StratconTrackState} in which the scenario occurs. If {@code null}, a
     *                              random trackState is selected.
     * @param template              The {@link ScenarioTemplate} for the scenario. If {@code null}, the default template
     *                              is used.
     * @param allowPlayerFacilities Whether the scenario is allowed to spawn on top of player-allied facilities.
    ...
    private static void applyFacilityModifiers(StratconScenario scenario, StratconTrackState track, StratconCoords coords) {
        ...
        for (StratconCoords facilityCoords : track.getFacilities().keySet()) {
            ...
            for (String modifierID : modifierIDs) {
                ...
                if (modifier == null) {
                    logger.error(String.format("Modifier %s not found for facility %s",
                          modifierID,
    ...
    /**
     * Helper function that determines if the unit type specified in the given scenario force template would start out
     * airborne on a ground map (hot dropped units aside)
     */
                ...
                if (track.getBackingScenariosMap().containsKey(backingScenario.getId())) {
                    ...
                    if (backingScenario.getStratConScenarioType().isLosTech()) {
                        if (victory) {
                            ...
                            StarLeagueCache cache = new StarLeagueCache(campaign,
                                  ((AtBContract) mission),
