 ...
 * infrastructure is destroyed on the spot: its objective is met, the contract's combat bonus is paid, and the
 * contract's Escalation rises by 3d6. If one does, its defenders ambush the attackers, in a template suited to
 * ambushing their unit type (see {@link AbstractStratConRolledPointOfInterestBehavior#getScenarioTemplateName()}): an
 * overall victory still destroys the infrastructure, meeting its objective, paying the bonus, and raising Escalation by
 * 2d6 - 3d6 in all, with the usual +1d6 for winning - while anything else - a defeat, a draw, or leaving the ambush
 * unplayed - fails it. (See {@link StratConContestedPointOfInterestBehavior} for the rules it shares, and
 * {@link StratConEscalation}.)</p>
 *
