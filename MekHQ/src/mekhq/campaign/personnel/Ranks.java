...
public class Ranks {
    ...
    private static final int[] officerCut = { /* SLDF */7, /* AFFS */6, /* AFFC */8, /* LCAF */14, /* LAAF */11, /* FWLM */9, /* CCAF */5, /* CCWH */2,
        /* DCMD */9, /* Clan */3, /* COM */2, /* WOB */2 };
    ...
    public static void initializeRankSystems() {
        ...
        MekHQ.getLogger().log(Ranks.class, METHOD_NAME, LogLevel.INFO, "Starting load of Rank Systems from XML..."); //$NON-NLS-1$
        ...
        MekHQ.getLogger().log(Ranks.class, METHOD_NAME, LogLevel.INFO, "Done loading Rank Systems"); //$NON-NLS-1$
    }
    ...
    public boolean isManeiDominiRelevant() {
        return rankSystem == RS_WOB;
    }
    ...
    public void writeToXml(PrintWriter pw1, int indent, boolean saveAll) {
        ...
        pw1.println(MekHqXmlUtil.indentStr(indent + 1) + "<system>" + rankSystem + "</system>");
    ...
    public static Ranks generateInstanceFromXML(Node wn, Version version) {
        ...
        try {
                    ...
                    if (version != null && Version.versionCompare(version, "0.3.4-r1782")) {
                        ...
                        if (retVal.rankSystem == RankTranslator.RT_SL) {
                            String change = (String) JOptionPane.showInputDialog(null,
                                ...
                                "Select Correct Rank System", JOptionPane.QUESTION_MESSAGE, null, RankTranslator.oldRankNames, RankTranslator.oldRankNames[0]);
            ...
            if (showMessage) {
                JOptionPane.showConfirmDialog(null, "You have used a custom rank set in your campaign." + "\nYou must recreate that system for this version.",
                    "Custom Ranks", JOptionPane.OK_CANCEL_OPTION);
            }
        } catch (Exception ex) {
    ...
    public void setRanksFromModel(RankTableModel model) {
        ...
        Vector row : vectors) {
            String[] names = { (String) row.get(RankTableModel.COL_NAME_MW), (String) row.get(RankTableModel.COL_NAME_ASF),
                (String) row.get(RankTableModel.COL_NAME_VEE), (String) row.get(RankTableModel.COL_NAME_NAVAL), (String) row.get(RankTableModel.COL_NAME_INF),
                (String) row.get(RankTableModel.COL_NAME_TECH) };
            Boolean officer = (Boolean) row.get(RankTableModel.COL_OFFICER);
            double payMult = (Double) row.get(RankTableModel.COL_PAYMULT);
            ranks.add(new Rank(Arrays.asList(names), officer, payMult));
        }
    }
}
