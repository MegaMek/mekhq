        ...
        if (isBoth || isEdge) {
            if (award.getEdgeReward() > 0) {
                if (isReplaceEdgeAwards) {
                    ...
                    int effectiveReward = award.getEdgeReward();
                    int maximumEdge = campaignOptions.get(CampaignOption.MAXIMUM_EDGE);
                    if (maximumEdge > 0) {
                        effectiveReward = Math.min(effectiveReward, maximumEdge - person.getEdge());
                    }
                    if (effectiveReward > 0) {
                        person.changeEdge(effectiveReward);
                        person.changeCurrentEdge(effectiveReward);
