...
public class Force {
    ...
    public int getTotalBV(Campaign campaign) {
        ...
        for (Force sforce : getSubForces()) {
            bvTotal += sforce.getTotalBV(campaign);
        }
        ...
        for (UUID id : getUnits()) {
            ...
            if (campaign.getCampaignOptions().isUseGenericBattleValue()) {
                bvTotal += campaign.getUnit(id).getEntity().getGenericBattleValue();
            } else {
                bvTotal += campaign.getUnit(id).getEntity().calculateBattleValue();
            }
        }
